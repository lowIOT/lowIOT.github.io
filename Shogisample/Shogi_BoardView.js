function Shogi_BoardView(app, ui_listener, cols, rows) {
...
Shogi_BoardView.prototype = Object.create(Shogi_Component.prototype);
Shogi_BoardView.prototype.constructor = Shogi_BoardView;

Shogi_BoardView.prototype.createElement = function() {
...
Shogi_BoardView.prototype.onRectSetted = function() {
...
Shogi_BoardView.prototype.setPieceImages = function(images) {
    this.piece_images = images;
}

Shogi_BoardView.prototype.setGame = function(game) {
    this.game = game;
}

Shogi_BoardView.prototype.update = function() {
    if (this.bg_img == null) {
        return;
    }
...
Shogi_BoardView.prototype.onElementClicked = function(x, y) {
...
Shogi_BoardView.prototype.setHolding = function(col, row) {
    this.holding = {'col': col, 'row': row};
    this.update();
}

Shogi_BoardView.prototype.resetHolding = function() {
    this.holding = null;
    this.update();
}

Shogi_BoardView.prototype.setMovableHands = function(hands) {
    this.movable_hands = hands;
    this.update();
}

Shogi_BoardView.prototype.resetMovableHands = function() {
    this.movable_hands = null;
    this.update();
}

Shogi_BoardView.prototype.setCheckedCell = function(cell) {
    this.checked_cell = cell;
    this.update();
}

Shogi_BoardView.prototype.resetCheckedCell = function() {
    this.checked_cell = null;
    this.update();
}

Shogi_BoardView.prototype.getLastMoved = function() {
    return this.last_moved;
}

Shogi_BoardView.prototype.setLastMoved = function(col, row) {
    this.last_moved = {'col': col, 'row': row};
}

Shogi_BoardView.prototype.resetLastMoved = function() {
    this.last_moved = null;
}

Shogi_BoardView.prototype.setLose = function(player) {
    var lose_col = -1;
    var lose_row = -1;
    for (var row = 0; row < this.rows; row++) {
        for (var col = 0; col < this.cols; col++) {
            if (this.game.board.data[col + row * this.cols] * player == Shogi_Piece.OU) {
                lose_col = col;
                lose_row = row;
                break;
            }
...
Shogi_BoardView.prototype.resetLose = function() {
    this.lose_cell = null;
}

Shogi_BoardView.prototype.reset = function() {
    this.resetHolding();
    this.resetHolding();
